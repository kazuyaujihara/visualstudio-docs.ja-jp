...
Azure Repos では、バージョン管理の 2 つのモデルを提供しています。分散バージョン管理である Git と、集中バージョン管理である Team Foundation バージョン管理 (TFVC) です。 この入門編は、Visual Studio for Mac で TFVC を使用する際の出発点としてご利用いただけます。
