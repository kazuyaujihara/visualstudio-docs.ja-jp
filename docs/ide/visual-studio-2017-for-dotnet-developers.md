...
Visual studio のキーボード ショートカットとコマンドの詳細については、[こちらのドキュメント](../ide/tips-and-tricks-for-visual-studio.md)を参照してください。
...
Visual Studio 2017 には、多くのリファクタリング、コード生成アクション、コード修正が含まれます。 赤の波線はエラー、緑の波線は警告、3 つのグレーの点はコード提案をそれぞれ表します。 コード修正にアクセスするには、電球またはねじ回しのアイコンをクリックするか、**Ctrl** + **.** キー または **Alt** + **Enter** キーを押します。 各修正にはプレビュー ウィンドウが付属しており、修正の効果によるライブ コードの相違が示されます。
