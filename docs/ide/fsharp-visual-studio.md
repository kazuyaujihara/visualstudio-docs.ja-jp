...
|ClickOnce|いいえ|別の .NET Framework 言語 (該当する場合) でクライアント プロジェクトを使用します。|
...
|.NET Framework API の定義へ移動|.NET Framework API にカーソルがある場合は、.NET Framework メタデータから生成されたコードが表示されます。|いいえ|
