...
- [Linux でのリモート デバッグ](debugging-python-code-on-remote-linux-machines.md)
...
| コマンド | 引数 | 説明 |
| --- | --- | --- |
| `$continue`では、 `$cont`では、 `$c` | 現在のステートメントからプログラムの実行を開始します。 |
...
| `$stepin`では、 `$step`では、 `$s` | 次の関数呼び出しにステップ インします (可能な場合)。 |
| `$stepout`では、 `$return`では、 `$r` | 現在の関数からステップ アウトします。 |
| `$stepover`では、 `$until`では、 `$unt` | 次の関数呼び出しにステップ オーバーします。 |
...
| `$where`では、 `$w`では、 `$bt` | 現在のスレッドのフレームを一覧表示します。 |
