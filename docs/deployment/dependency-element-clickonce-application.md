...
### <a name="dependentos"></a>dependentOS
 任意。 含まれています、`osVersionInfo`要素。 `dependentOS`と`dependentAssembly`要素は相互に排他的: のいずれか 1 つが存在する必要があります、`dependency`両方ではなく、要素。
...
### <a name="dependentassembly"></a>dependentAssembly
 任意。 含まれています、`assemblyIdentity`要素。 `dependentOS`と`dependentAssembly`要素は相互に排他的: のいずれか 1 つが存在する必要があります、`dependency`両方ではなく、要素。
...
| `allowDelayedBinding` | 必須です。 実行時にプログラムによって読み込むアセンブリことができるかどうかを指定します。 |
...
### <a name="hash"></a>hash
 `hash`要素のオプションの子では、`assemblyIdentity`要素。 `hash` 要素に属性はありません。
