...
## <a name="example"></a>例
次の例は、セキュリティで保護されていないフィールドを持つライブラリの種類 (`SecuredTypeWithFields`)、ライブラリ型のインスタンスを作成できる型 (`Distributor`)、および型に対して、インスタンスを作成するためのアクセス許可を持たない型 ()、およびを読み取ることができるアプリケーションコードで構成されています。インスタンスのフィールドには、型をセキュリティで保護するアクセス許可がありません。
