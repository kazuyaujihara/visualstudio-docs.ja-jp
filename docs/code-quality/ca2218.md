...
## <a name="cause"></a>原因
パブリック型は <xref:System.Object.Equals%2A?displayProperty=fullName> よりも優先されますが、<xref:System.Object.GetHashCode%2A?displayProperty=fullName> をオーバーライドしません。
...
この規則違反を修正するには、<xref:System.Object.GetHashCode%2A> の実装を指定します。 同じ型のオブジェクトのペアについては、<xref:System.Object.Equals%2A> の実装でペアに `true` が返される場合は、実装が同じ値を返すようにする必要があります。
