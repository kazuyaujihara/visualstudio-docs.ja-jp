...
例外がスローされると、それに含まれる情報の一部がスタックトレースになります。 スタックトレースは、例外をスローするメソッドで始まり、例外をキャッチするメソッドで終了するメソッド呼び出し階層のリストです。 @No__t-0 ステートメントで例外を指定して例外が再スローされた場合、現在のメソッドでスタックトレースが再開され、例外をスローした元のメソッドと現在のメソッドとの間のメソッド呼び出しの一覧が失われます。 例外と共に元のスタックトレース情報を保持するには、例外を指定せずに `throw` ステートメントを使用します。
