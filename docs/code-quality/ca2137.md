...
## <a name="example"></a>例
この例のメソッドでは、検証不可能なコードを使用しており、<xref:System.Security.SecurityCriticalAttribute> または <xref:System.Security.SecuritySafeCriticalAttribute> の属性でマークする必要があります。
