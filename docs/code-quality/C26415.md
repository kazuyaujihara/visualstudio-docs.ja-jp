...
コアガイドライン: r. 30: 有効期間のセマンティクスを明示的に表現するためにのみ、スマートポインターをパラメーターとして取得します。 **C++**
...
このチェックでは、C26410、C26415、C26417、C26418 の原因となるほとんどのシナリオについて説明します。 最初に SMART_PTR_NOT_NEEDED をクリーンアップしてから、共有ポインターまたは一意ポインターのエッジケースに切り替えることをお勧めします。 さらに焦点を絞ったクリーンアップでは、この警告を無効にすることができます。

このチェックでは、標準の std:: unqiue_pointer および std:: shared_pointer テンプレートに加えて、スマートポインターとして使用される可能性のあるユーザー定義型を認識します。 このような型では、次の操作を定義する必要があります。
