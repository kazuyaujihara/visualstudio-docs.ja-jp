...
コンポーネントオブジェクトモデル (COM) クライアントで .NET 型を作成するには、その型を最初に登録する必要があります。 使用可能な場合は、ユーザー指定のコードを実行するための登録プロセス中に、<xref:System.Runtime.InteropServices.ComRegisterFunctionAttribute> 属性でマークされたメソッドが呼び出されます。 @No__t_0 属性でマークされた対応するメソッドは、登録メソッドの操作を元に戻すための登録解除プロセス中に呼び出されます。
