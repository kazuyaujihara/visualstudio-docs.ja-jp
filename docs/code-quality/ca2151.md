...
透過的セキュリティ フィールドまたはセーフ クリティカル フィールドが宣言されました。 その型は、セキュリティ クリティカルとして指定されています。 以下に例を示します。
...
この規則違反を修正するには、フィールドを @no__t 0 属性でマークするか、フィールドによって参照される型をセキュリティ transparent または safe critical のいずれかに設定します。
