...
この規則違反を修正するには、アセンブリを読み込んでいるメソッドを <xref:System.Security.SecurityCriticalAttribute> または <xref:System.Security.SecuritySafeCriticalAttribute> の属性でマークします。
