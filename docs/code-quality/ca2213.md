...
- @No__t-0 の呼び出しが、ルールチェックよりも深い呼び出しレベルで発生します。
...
次のスニペットは、フィールド `aFieldOfADisposableType` を破棄可能な型 (`TypeA`) として宣言し、フィールドで <xref:System.IDisposable.Dispose%2A> を呼び出さないことによって、rule CA2213 に違反する型 @no__t を示しています。
