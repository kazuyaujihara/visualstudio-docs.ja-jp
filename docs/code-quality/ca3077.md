...
- <xref:System.Xml.XmlDocument> または <xref:System.Xml.XmlTextReader> のクラスは、DTD 処理に既定の競合回避モジュールの値を使用します。
...
- XmlTextReader がアクセスできるリソースを制限するには、XmlResolver ではなく <xref:System.Xml.XmlSecureResolver>instead 使用します。
