...
パブリックコンストラクターに別のアクセスレベルを使用することによって違反が発生した場合に、この規則からの警告を抑制するのは安全です。 また、ポータブルクラスライブラリ (PCL) を構築している場合は、`NewException(SerializationInfo, StreamingContext)` コンストラクターの警告を抑制することもできます。
