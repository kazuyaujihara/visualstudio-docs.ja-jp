...
@No__t-0 をオーバーライドする型は、基本クラスで <xref:System.Object.Finalize%2A> メソッドを呼び出しません。
...
この規則違反を修正するには、基本データ型の <xref:System.Object.Finalize%2A> メソッドを <xref:System.Object.Finalize%2A> メソッドから呼び出します。
