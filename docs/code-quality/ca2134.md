...
## <a name="cause"></a>原因
この規則は、@no__t でマークされたメソッドが、透過的または <xref:System.Security.SecuritySafeCriticalAttribute> でマークされたメソッドをオーバーライドするときに適用されます。 この規則は、透過的または @no__t でマークされたメソッドが、<xref:System.Security.SecurityCriticalAttribute> でマークされたメソッドをオーバーライドした場合にも発生します。
...
この規則からの警告を抑制しないでください。 この規則に違反すると、レベル2の透過性を使用するアセンブリに対して、実行時 <xref:System.TypeLoadException> になります。
