...
最初にオペランドを検証することなく算術演算を実行して、操作の結果が関連するデータ型の有効な値の範囲を超えていないことを確認してください。 算術オーバーフローで<xref:System.OverflowException?displayProperty=fullName>は、実行コンテキストおよび関連するデータ型に応じて、または結果の最上位ビットが破棄される可能性があります。
...
この例のメソッドが渡さ<xref:System.Int32.MinValue?displayProperty=fullName>れた場合、操作はアンダーフローします。 これにより、結果の最上位ビットが破棄されます。 この状況を次のコードに示します。
...
次の例では、チェックされたブロックで操作をラップすることによって、前の違反を修正します。 操作によってオーバーフロー <xref:System.OverflowException?displayProperty=fullName>が発生した場合は、がスローされます。
