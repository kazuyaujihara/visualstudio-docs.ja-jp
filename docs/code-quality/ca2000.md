...
@No__t 0 の型のローカルオブジェクトが作成されましたが、オブジェクトへのすべての参照がスコープ外になる前に、オブジェクトが破棄されていません。
...
[@No__t-1 ステートメント](/dotnet/csharp/language-reference/keywords/using-statement)(Visual Basic の[`Using`](/dotnet/visual-basic/language-reference/statements/using-statement) ) を使用して、<xref:System.IDisposable> を実装するオブジェクトをラップすることができます。 このようにラップされたオブジェクトは、`using` ブロックの最後に自動的に破棄されます。 ただし、次の状況は、`using` ステートメントでは処理できません。

- 破棄可能なオブジェクトを返すには、オブジェクトを `using` ブロックの外側の @no__t 0 ブロックで構築する必要があります。

- @No__t-0 ステートメントのコンストラクターで、破棄可能なオブジェクトのメンバーを初期化しないでください。
...
- 動的オブジェクトは、shadow オブジェクトを使用して @no__t 0 オブジェクトの dispose パターンを実装する必要があります。

## <a name="when-to-suppress-warnings"></a>警告を非表示にする場合

次の場合を除き、この規則からの警告を抑制しないでください。

- @No__t-1 などの `Dispose` を呼び出すメソッドをオブジェクトで呼び出しました。
- 警告を発生させたメソッドは、オブジェクトをラップする @no__t 0 のオブジェクトを返します。
...
- [CA2202:オブジェクトを複数回破棄することはできません @ no__t-0
