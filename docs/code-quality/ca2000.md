...
@No__t_0 型のローカルオブジェクトが作成されましたが、オブジェクトへのすべての参照がスコープ外になる前に、オブジェクトが破棄されていません。
...
[@No__t_1 ステートメント](/dotnet/csharp/language-reference/keywords/using-statement)(Visual Basic の[`Using`](/dotnet/visual-basic/language-reference/statements/using-statement) ) を使用して、<xref:System.IDisposable> を実装するオブジェクトをラップすることができます。 このようにラップされたオブジェクトは、`using` ブロックの最後に自動的に破棄されます。 ただし、次の状況は、`using` ステートメントでは処理できません。

- 破棄可能なオブジェクトを返すには、オブジェクトを `using` ブロックの外側の `try/finally` ブロックで構築する必要があります。

- @No__t_0 ステートメントのコンストラクターで、破棄可能なオブジェクトのメンバーを初期化しないでください。
