...
## <a name="cause"></a>原因
@No__t を実装する型は、<xref:System.IDisposable> も実装する型から継承されます。 継承する型の <xref:System.IDisposable.Dispose%2A> メソッドは、親の型の <xref:System.IDisposable.Dispose%2A> メソッドを呼び出しません。
