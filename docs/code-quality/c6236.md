...
## <a name="example"></a>例
 このコードは、警告 C6236 を表示する方法を示しています。 @No__t_0 が0ではないため、`n || INPUT_TYPE` 式は常に0以外であり、`else` 句は実行されません。 ただし、`INPUT_TYPE` は1以外の値を持つ定数であり、比較のための値として指定されています。
