...
- @No__t_0 プロパティが信頼できるソースから割り当てられていることを確認します。
...
- @No__t_0 プロパティを **[禁止]** または **[無視]** に設定して、信頼されていないソースを扱う場合は、DtdProcessing を有効にしないでください。
...
> [!NOTE]
> @No__t_1 インスタンスを作成する方法として <xref:System.Xml.XmlReader.Create%2A?displayProperty=nameWithType> が推奨されていますが <xref:System.Xml.XmlTextReader> との動作の違いがあります。 @No__t_1 の <xref:System.Xml.XmlReader> は、XML 値の `\n` に `\r\n` を正規化しますが、<xref:System.Xml.XmlTextReader> は `\r\n` シーケンスを保持します。
