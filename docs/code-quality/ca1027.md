...
次の例では、`DaysEnumNeedsFlags` は <xref:System.FlagsAttribute> を使用するための要件を満たしていますが、それを持たない列挙体です。 @No__t_0 列挙体には、2の累乗ではなく <xref:System.FlagsAttribute> を誤って指定した値が含まれていません。 これは rule [CA2217 に違反します: FlagsAttribute で列挙をマークしません](../code-quality/ca2217.md)。
