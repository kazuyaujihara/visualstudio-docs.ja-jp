...
次の例のメソッドは、この規則に違反する整数を操作します。 [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] にするには、このを起動するために [整数オーバーフローを**削除**する] オプションを無効にする必要があります。
...
チェックされたブロックは @no__t 0 ではサポートされていません。
