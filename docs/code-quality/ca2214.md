...
> [!NOTE]
> この規則のレガシ分析実装には、"\[ コンストラクター名" という異なる診断メッセージがあり、**クラスによって定義された仮想メソッドが呼び出される呼び出しチェーンが含まれています。意図しない結果を得るために、次の呼び出し履歴を確認**します。 この規則の[FxCop アナライザー](install-fxcop-analyzers.md)の実装には、"**コンストラクターでオーバーライド可能なメソッドを呼び出さない**" という診断メッセージがあります。
