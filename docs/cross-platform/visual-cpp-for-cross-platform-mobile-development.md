...
- **統合されたデバッグ操作。** Visual Studio のワールドクラスのデバッグ ツールを使用して、次のようなすべてのプラットフォームで C++ コードを観察しながらステップ実行できます: Android のデバイスとエミュレーター、iOS のシミュレーターとデバイス、Windows または Windows Store のデバイスとエミュレーター。
