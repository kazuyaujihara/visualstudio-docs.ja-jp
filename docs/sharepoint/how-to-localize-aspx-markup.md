...
4.  ASPX マークアップに加えてコードをローカライズするリソースを使用する場合の値を受け入れ、**ビルド アクション**プロパティとして各ファイルの**埋め込まれたリソース**します。 マークアップのローカライズにのみ、リソース ファイルを使用する場合は、ファイルのプロパティの値を必要に応じて変更できます**コンテンツ**します。 詳細については、[ローカライズ SharePoint ソリューション](../sharepoint/localizing-sharepoint-solutions.md)を参照してください。
