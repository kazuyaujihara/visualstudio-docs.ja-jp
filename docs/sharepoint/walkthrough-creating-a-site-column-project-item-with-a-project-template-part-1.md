...
2. SiteColumnProjectItemTypeProvider コード ファイルでは、コード内の最初の行にブレークポイントを追加、`InitializeType`メソッドを選択し、 **F5**デバッグを開始するキー。
