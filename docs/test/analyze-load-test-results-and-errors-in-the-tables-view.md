...
|**キャッシュ済み**|既にキャッシュされていた要求の合計回数です。|いいえ|
|**要求/秒**|ロード テストの実行中の 1 秒あたりの要求数です。|いいえ|
|**成功 (秒単位)**|ロード テストの実行中にこの要求が成功した 1 秒あたりの頻度です。|いいえ|
|**失敗 (秒単位)**|ロード テストの実行中にこの要求が失敗した 1 秒あたりの頻度です。|いいえ|
|**先頭バイトまでの時間**|応答の先頭バイトを受信するまでの平均時間です。要求が Web サーバーに送信された時点から計測されます。 単位は秒です。|いいえ|
...
|**成功 (秒単位)**|ロード テストの実行中にこのテストが成功した 1 秒あたりの頻度です。|いいえ|
|**失敗 (秒単位)**|ロード テストの実行中にこのテストが失敗した 1 秒あたりの頻度です。|いいえ|
|**テスト時間**|ロード テストの実行中に、テストの実行にかかった平均時間です。 単位は秒です。|はい|
|**90% テスト時間**|テスト時間の 90 パーセンタイル値です。|いいえ|
|**95% テスト時間**|テスト時間の 95 パーセンタイル値です。|はい|
|**要求 (テスト毎)**|Web パフォーマンス テストの場合に、テストに含まれていた要求の平均数です。|いいえ|
...
|-|-|-|
|**トランザクション**|トランザクションの名前です。|はい|
|**シナリオ**|シナリオの名前です。|はい|
|**テスト**|テストの名前です。|はい|
|**合計**|ロード テスト中に発行されたトランザクションの合計数です。|はい|
|**トランザクション時間**|ロード テストの実行中に、トランザクションの実行にかかった時間です。 Web パフォーマンス テストの場合は、待ち時間も計算に含まれます。 単位は秒です。|いいえ|
|**応答時間**|ロード テストの実行における Web パフォーマンス テスト トランザクションの応答時間です。 応答時間がトランザクション時間と異なる点として、応答時間にはトランザクション中に発生した待ち時間が含まれません。 単位は秒です。|いいえ|
|**平均トランザクション時間**|トランザクションの平均時間です。 この時間には待ち時間が含まれます。 たとえば、3 つの要求があり、それぞれの要求に待ち時間がある場合、この時間には 3 つの要求の待ち時間と、要求を実行するための実際の時間が含まれます。|いいえ|
