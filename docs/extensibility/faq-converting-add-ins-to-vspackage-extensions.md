...
## <a name="how-do-i-set-keyboard-shortcuts-in-a-vspackage"></a>VSPackage でキーボード ショートカットを設定するにはどうしたらよいですか?
 使用する、`<KeyBindings>`の要素、 *.vsct*ファイル。 次の例では、コマンドのキーボード ショートカット`idCommand1`は**Alt**+**A**、およびコマンドのキーボード ショートカット`idCommand2`は**Alt** + **Ctrl**+**A**します。 キー名の構文に注意してください。
