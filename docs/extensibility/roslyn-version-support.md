---
title: サポートされている Roslyn パッケージ バージョンのマッピング
ms.date: 04/29/2019
ms.topic: reference
helpviewer_keywords:
- roslyn package versions
author: gewarren
ms.author: gewarren
manager: jillfra
ms.workload:
- vssdk
ms.openlocfilehash: 3b2dd97b078923cfa3358d56e6316bfff654c4dd
ms.sourcegitcommit: 62f42113ae4dae1ddfff1c4e02445acc09913445
ms.translationtype: MT
ms.contentlocale: ja-JP
ms.lasthandoff: 04/29/2019
ms.locfileid: "64878238"
---
# <a name="net-compiler-platform-package-version-reference"></a>.NET コンパイラ プラットフォーム パッケージのバージョンのリファレンス

次の表では[.NET コンパイラ プラットフォーム (Roslyn) パッケージ](https://www.nuget.org/packages/Microsoft.Net.Compilers/)さまざまなバージョンの Visual Studio のバージョンがサポートされています。

たとえば、カスタム アナライザーが Visual Studio 2017 でのすべてのバージョンで動作するようにする必要がありますを対象と Microsoft.Net.Compilers のバージョン 2.0。

| Roslyn パッケージ バージョン | サポートされる最小バージョンの Visual Studio |
| - | - |
| 3.x | Visual Studio 2019 |
| 2.10.0 | Visual Studio 2017 バージョン 15.9 |
| 2.9.0 | Visual Studio 2017 バージョン 15.8 |
| 2.8.2 | Visual Studio 2017 バージョン 15.7 |
| 2.7.0 | Visual Studio 2017 バージョン 15.6 |
| 2.6.1 | Visual Studio 2017 バージョン 15.5 |
| 2.4.0 | Visual Studio 2017 バージョン 15.4 |
| 2.3.2 | Visual Studio 2017 バージョン 15.3 |
| 2.2.0 | Visual Studio 2017 バージョン 15.2 |
| 2.1.0 | Visual Studio 2017 バージョン 15.1 |
| 2.0.0 | Visual Studio 2017 RTM |
| 1.3.2 | Visual Studio 2015 update 3 |
| 1.2.2 | Visual Studio 2015 update 2 |
| 1.1.1 | Visual Studio 2015 update 1 |
| 1.0.1 | Visual Studio 2015 RTM |

> [!TIP]
> Roslyn のパッケージが最小のサポートされている Visual Studio のバージョンには、Visual Studio 2017 バージョンですべてのバージョンの Visual Studio 2019 は後で付属しているためもサポートされます。

## <a name="see-also"></a>関連項目

- [.NET Compiler Platform SDK](/dotnet/csharp/roslyn-sdk/)
- [Roslyn アナライザーを概要します。](getting-started-with-roslyn-analyzers.md)