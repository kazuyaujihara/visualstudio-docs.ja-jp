...
プロジェクトがでまとめて収集することも可能性があります、*ソリューション*、これは、連携してアプリケーションを作成する 1 つまたは複数のプロジェクトのグループ化します。 2 つのソリューション ファイル、テキスト ベースのソリューションに関連するプロジェクトとステータスの情報が格納されている[ソリューション (.sln) ファイル](solution-dot-sln-file.md)と二項[ソリューション ユーザー オプション (.suo) ファイル](solution-user-options-dot-suo-file.md)します。 これらのファイルは Visual Basic の以前のバージョンで使用されていたグループ (.vbg) ファイルに似ており、ワークスペース (.dsw) とユーザーのオプション (.opt) ファイルの以前のバージョンで使用されていたC++します。
