...
### <a name="activate-a-solution-load-manager"></a>ソリューション読み込みのマネージャーをアクティブ化します。
 ...
 取得する必要があります、<xref:Microsoft.VisualStudio.Shell.Interop.SVsSolution>サービスし、設定、 [__VSPROPID4 します。VSPROPID_ActiveSolutionLoadManager](<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID4.VSPROPID_ActiveSolutionLoadManager>)プロパティ。
...
-   <xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionLoadEvents.OnBeforeBackgroundSolutionLoadBegins%2A>:解決するには、完全に読み込まれるが、バック グラウンドの前にプロジェクトの読み込みをもう一度開始した後、このイベントが発生します。

-   <xref:Microsoft.VisualStudio.Shell.Interop.IVsSolutionLoadEvents.OnAfterBackgroundSolutionLoadComplete%2A>:このイベントは発生ソリューションが最初に完全に読み込まれた後、ソリューション ロードのマネージャーがあるかどうか。 これは、ソリューションが完全に読み込まれるたびに、バック グラウンド負荷または需要が読み込まれた後も発生します。 同時に、<xref:Microsoft.VisualStudio.VSConstants.UICONTEXT.SolutionExistsAndFullyLoaded_guid>が再アクティブ化します。
...
- [__VSPROPID4 します。VSPROPID_IsSolutionFullyLoaded](<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID4.VSPROPID_IsSolutionFullyLoaded>):`var`返します`true`ソリューションとそのすべてのプロジェクトが読み込まれている場合、それ以外の場合`false`します。

- [__VSPROPID4 します。VSPROPID_IsInBackgroundIdleLoadProjectBatch](<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID4.VSPROPID_IsInBackgroundIdleLoadProjectBatch>):`var`返します`true`かどうかのプロジェクトのバッチが現在読み込まれる、バック グラウンドでそれ以外の場合`false`します。

- [__VSPROPID4 します。VSPROPID_IsInSyncDemandLoadProjectBatch](<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID4.VSPROPID_IsInSyncDemandLoadProjectBatch>):`var`返します`true`かどうかプロジェクトのバッチが現在が読み込まれる同期的に結果として、ユーザー コマンドやその他の明示的な読み込み、それ以外の場合`false`します。

- [__VSPROPID2 します。VSPROPID_IsSolutionClosing](<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID2.VSPROPID_IsSolutionClosing>):`var`返します`true`ソリューションが現在されて閉じられた場合、それ以外の場合`false`します。

- [__VSPROPID します。VSPROPID_IsSolutionOpening](<xref:Microsoft.VisualStudio.Shell.Interop.__VSPROPID.VSPROPID_IsSolutionOpening>):`var`返します`true`ソリューションが現在いる開かれた場合、それ以外の場合`false`します。
