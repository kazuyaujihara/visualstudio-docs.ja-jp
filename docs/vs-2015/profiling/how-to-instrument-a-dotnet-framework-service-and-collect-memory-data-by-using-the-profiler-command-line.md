...
## <a name="starting-the-profiling-session"></a>プロファイル セッションの開始  
 ...
 サービスをホストするコンピューターをプロファイリング用に構成するには再起動が必要です。 サービス コントロール マネージャーからサービスを手動で起動することも必要です。 その後、プロファイラーを起動し、[!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)] サービスを起動します。  
