---
title: プログラムコードでのモデルの移動と更新 |Microsoft Docs
ms.date: 11/15/2016
...
## <a name="in-this-topic"></a>このトピックの内容
 ...
 [トランザクション内で変更を実行する](#transaction)

 [モデル要素の作成](#elements)

 [リレーションシップリンクの作成](#links)

 [要素の削除](#deleteelements)

 [リレーションシップリンクの削除](#deletelinks)
 ...
 [ダイアグラムの移動と更新](#diagrams)

 [図形と要素間の移動](#views)

 [図形とコネクタのプロパティ](#shapeProperties)

 [DocView と Docview](#docdata)

 図形、コネクタ、図、およびモデル要素との関係については、別のトピックで説明します。 詳細については、「[方法: ダイアグラムを移動および更新する](../misc/how-to-navigate-and-update-a-diagram.md)」を参照してください。
 ...
 **非表示のロール。** DSL 定義では、特定のロールに対して **"プロパティが生成**されました" が**false**の場合、そのロールに対応するプロパティは生成されません。 ただし、リレーションシップのメソッドを使用してリンクにアクセスし、リンクを走査することはできます。
 ...
 最もよく使用される例は、モデル要素を図に表示する図形にリンクする、<xref:Microsoft.VisualStudio.Modeling.Diagrams.PresentationViewsSubject> リレーションシップです。
...
- 新しい要素のプロパティを設定します。特に、DslDefinition で `IsName` が true であるプロパティを設定します。 このフラグは、自身の所有者内で要素を一意に識別するために機能するプロパティをマークします。 この例では、Name プロパティにそのフラグが指定されています。
...
- @No__t_0 フラグが true であるロールの要素。 たとえば、要素を表示する図形は削除されます。
  ...
  既定では、埋め込みリレーションシップはすべて、ターゲットロールで `PropagatesDelete` true になります。 @No__t_0 を削除しても `familyTree` は削除されませんが、`familyTree.Delete()` によってすべての `Persons` が削除されます。 詳細については、「[削除動作のカスタマイズ](../modeling/customizing-deletion-behavior.md)」を参照してください。
...
### <a name="presentation-elements"></a>プレゼンテーション要素
 ![基本図形と要素の種類のクラス図](../modeling/media/dslshapesandelements.png "DSLshapesAndElements")
...
|図|<xref:Microsoft.VisualStudio.Modeling.Diagrams.Diagram>|
 ...
 図形は、2つのセットに子図形を持つことができます。 @No__t_0 セット内の図形は、その親の境界ボックスに限定されます。 @No__t_0 リスト内の図形は、親の境界の外側または一部に表示できます (ラベルやポートなど)。 図には `RelativeChildShapes` がなく、`Parent` もありません。
...
- <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.StyleSet%2A>-図形またはコネクタを描画するために使用されるペンとブラシのセット

- <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.Hide%2A>-図形を非表示にします

- <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.Show%2A>-`Hide()` の後に図形を表示します
...
- DSL 定義で指定したすべての要素マージディレクティブを監視します。
  ...
  この例では、ユーザーが図をダブルクリックしたときに、マウスの位置に図形を作成します。 このサンプルの DSL 定義では、`ExampleShape` の `FillColor` プロパティが公開されています。
...
### <a name="use-transactions"></a>トランザクションの使用
 図形、コネクタ、および図は、ストア内で <xref:Microsoft.VisualStudio.Modeling.ModelElement> およびライブのサブタイプです。 したがって、トランザクション内でのみ変更を加える必要があります。 詳細については、「[方法: トランザクションを使用](../modeling/how-to-use-transactions-to-update-the-model.md)してモデルを更新する」を参照してください。

## <a name="docdata"></a>ドキュメントビューとドキュメントデータ
 ![標準の図の種類のクラス図](../modeling/media/dsldiagramsanddocs.png "DSLDiagramsandDocs")
...
## <a name="see-also"></a>参照
 ドメイン固有[言語での検証](../modeling/validation-in-a-domain-specific-language.md)の <xref:Microsoft.VisualStudio.Modeling.ModelElement>[ドメイン固有言語からコードを生成](../modeling/generating-code-from-a-domain-specific-language.md)する[方法: トランザクションを使用してモデルを更新する](../modeling/how-to-use-transactions-to-update-the-model.md) [Visual Studio modelbus を使用](../modeling/integrating-models-by-using-visual-studio-modelbus.md)してモデルを統合する[変更の反映](../modeling/responding-to-and-propagating-changes.md)
