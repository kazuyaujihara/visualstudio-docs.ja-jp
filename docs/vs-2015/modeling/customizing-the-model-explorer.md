...
## <a name="changing-the-tab-icon"></a>タブアイコンの変更
 エクスプローラーのタブアイコンを変更するには、.bmp ファイルで16x16 ピクセルのアイコンを使用します。 アイコンファイルを ModelExplorerToolWindowBitmaps フォルダーに保存してから、ファイル名を「」に変更します。 たとえば、[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] の .ico アイコンファイルを .bmp 形式に変更し、名前を**DSLLanguageName\DslPackage\Resources\ModelExplorerToolWindowBitmaps.bmp**に変更します。 生成されたデザイナーは、**ソリューションエクスプローラー**と共にドッキングされると、このアイコンがエクスプローラーのタブに表示されます。
...
1. タスクフローソリューションテンプレートを使用して、[!INCLUDE[dsl](../includes/dsl-md.md)] ソリューションを作成します。
