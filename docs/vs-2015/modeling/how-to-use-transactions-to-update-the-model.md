...
トランザクションでは、ストアに加えられた変更がグループとして扱われるようにします。 グループ化された変更は、1つの単位としてコミットまたはロールバックできます。
 ...
 プログラムコードによって [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] の視覚化およびモデリング SDK でストアの要素が変更、追加、または削除されるたびに、トランザクション内でそれを行う必要があります。 変更が発生したときに、ストアに関連付けられている <xref:Microsoft.VisualStudio.Modeling.Transaction> のアクティブなインスタンスが存在している必要があります。 これは、すべてのモデル要素、リレーションシップ、図形、図、およびそれらのプロパティに適用されます。
