...
## <a name="multi-processor-logging-challenges"></a>マルチプロセッサ ログの問題点  
 1 つ以上のプロジェクトをマルチプロセッサ システムまたはマルチコア システムでビルドすると、すべてのプロジェクトの [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] ビルド イベントが同時に生成されます。 大量のイベント メッセージが同時に、または誤った順序で logger に送られてくる可能性があります。 [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] 2.0 の logger はこのような状況には対応していないため、logger が過負荷となり、ビルド時間の増加や不正確な logger 出力をもたらすだけでなく、ビルドが破損することもあります。 これらの問題を解決するために、[!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] 3.5 以降の logger は順序が誤っているイベントを処理し、イベントとそのソースを対応付けます。  
...
## <a name="multi-processor-logging-models"></a>マルチプロセッサ ログ モデル  
 マルチプロセッサに起因するビルド関連の問題に対処するために、[!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] は中央ログ モデルと分散ログ モデルという 2 つのログ モデルをサポートしています。  
...
### <a name="distributed-logging-model"></a>分散ログ モデル  
 中央ログ モデルでは、一度に多数のプロジェクトをビルドする場合など、大量の受信メッセージ トラフィックが発生し、中央ノードが過負荷となることがあります。 その結果、システム リソースの負荷増大やビルド パフォーマンスの低下を招くことがあります。 この問題を軽減するために、[!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] は分散ログ モデルをサポートしています。  
