 ...
 **例外が AppDomain またはマネージド/ネイティブ境界を越える場合にブレークします。**  
 ...
 1\)ネイティブ コードは、COM 相互運用機能を使用してマネージ コードを呼び出し、マネージ コードが例外をスローします。 参照してください[COM 相互運用機能の概要](https://msdn.microsoft.com/library/8bd62e68-383d-407f-998b-29aa0ce0fd67)します。  
  
 2\) 1 のアプリケーション ドメインで実行されるマネージ コードは、アプリケーション ドメイン 2、マネージ コードを呼び出し、アプリケーション ドメイン 2 のコードが例外をスローします。 参照してください[Programming with Application Domains](https://msdn.microsoft.com/bd36055b-56bd-43eb-b4d8-820c37172131)します。  
  
 3\)コードがリフレクションを使用して関数を呼び出し、関数が例外をスローします。 参照してください[リフレクション](https://msdn.microsoft.com/library/d1a58e7f-fb39-4d50-bf84-e3b8f9bf9775)します。  
 ...
 **例外処理アシスタントを有効にします。**  
 マネージド コード専用です。 マネージド例外は [例外処理アシスタント] ダイアログ ボックスを開きます。  参照してください[例外処理アシスタント](https://msdn.microsoft.com/library/992892ac-9d52-44cc-bf09-b44bfc5befeb)します。  
