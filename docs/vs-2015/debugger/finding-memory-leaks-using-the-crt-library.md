...
- ブロックのデータの最初の 16 バイト (16 進形式)  
  ...
  これらの手法は、標準 CRT の `malloc` 関数を使用して割り当てられたメモリに適用されます。 プログラムが、C++ を使用してメモリを割り当てる場合`new`演算子、ただし、のみが確認できますファイルと行番号、グローバルの実装`operator new`呼び出し`_malloc_dbg`メモリ リーク レポートにします。 その動作が非常に便利なため、次のようなマクロを使用して、割り当てが行われた行を報告するように変更できます。 
...
置き換えることができますので、`new`演算子を使用して、`DBG_NEW`コードでマクロ。 グローバルのオーバー ロードを使用するデバッグ ビルドでこの`operator new`ブロックの型、ファイル、および行番号の追加のパラメーターを受け取る。 このオーバー ロード`new`呼び出し`_malloc_dbg`余分な情報を記録します。 使用すると`DBG_NEW`、メモリ リーク レポートが、リークしているオブジェクトが割り当てられたファイル名と行番号が表示されます。 既定値を使用して、製品版ビルドで`new`します。 (という名前のプリプロセッサ マクロを作成しないで`new`、またはその他の言語のキーワードです)。次の手法の例に示します。  
...
これは、リークした割り当てが debug_new.cpp の 20 行目にしたことです。  
...
3. **ウォッチ**ウィンドウで、「`_crtBreakAlloc`で、**名前**列。  
