...
警告 C6279:\<変数 > 使用が割り当てられたスカラー new、配列 delete で削除  
 ...
 のみこの警告が表示されますC++コードし、呼び出し元の関数が、スカラー型のメモリを割り当て一貫性がないことを示します**新しい**演算子、解放しますが、配列で**delete[]** 演算子。 スカラーでメモリが割り当てられた場合**新しい**、スカラーを解放する必要が通常**削除**します。  
