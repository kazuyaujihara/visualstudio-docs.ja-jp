...
警告 C6278:\<変数 > は配列 new を使用して割り当てられたが、スカラー delete で削除します。 デストラクターは呼び出されません  
 ...
 のみこの警告が表示されますC++コードし、呼び出し元の関数が、配列にメモリを割り当て一貫性がないことを示します**new[]** 演算子、解放しますが、スカラー型の**削除**演算子。 C++ 標準および Microsoft Visual C++ の実装によると、これは未定義の動作です。 これによって問題が生じる原因は、少なくとも 3 つあります。  
