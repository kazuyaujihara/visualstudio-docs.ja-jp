...
3. アプリケーションの 1 つのバージョンのみ使用できるように発行バージョンをリセットします。 既定では、アプリケーションを発行するたびに発行バージョンのリビジョン番号が自動的にインクリメントされます。 詳細については、「[方法 :発行バージョンを設定、ClickOnce](../deployment/how-to-set-the-clickonce-publish-version.md)します。  
