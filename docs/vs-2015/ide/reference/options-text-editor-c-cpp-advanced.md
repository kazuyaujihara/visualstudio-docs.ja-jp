 ...
 **#Include オートコンプリートを無効にする**@No__t_1 ステートメントのオートコンプリートを無効にします。

 **オートコンプリート #include にスラッシュを使用する**"/" が使用されている場合に、`#include` ステートメントのオートコンプリートをトリガーします。 既定の区切り記号は円記号 '\' です。 コンパイラでは、どちらの記号も使用できるため、このオプションを使用して、コード ベースが使用する記号を指定します。
 ...
 **メンバー一覧のキーワードを無効にする**@No__t_1、`class`、`switch` などの言語キーワードは、メンバー一覧の提案には表示されません。
...
## <a name="see-also"></a>参照
 [言語固有のフォルダー オプションの設定](../../ide/reference/setting-language-specific-editor-options.md)
