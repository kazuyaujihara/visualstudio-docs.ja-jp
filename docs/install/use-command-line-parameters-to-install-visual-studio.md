...
| `--channelUri <uri>` | **省略可能**:チャネル マニフェストの URI です。 更新が不要な場合、`--channelUri` によって、存在しないファイル (たとえば、--channelUri C:\doesntExist.chman) を指定することができます。 インストール コマンドに使用できます。その他のコマンドでは無視されます。 |
