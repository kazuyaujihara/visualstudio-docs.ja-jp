...
このエラーは、リモート デバッガー サービスが、デバッグを実行しているコンピューターに接続するときに認証できないユーザー アカウントで実行されていることを意味します。 リモート デバッグの従来のデバッグ エンジンを使用するときに、このエラーが発生する可能性があり、リモート デバッガーをサービスとして実行します。
