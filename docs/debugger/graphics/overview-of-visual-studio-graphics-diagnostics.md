...
### <a name="pipeline-stages"></a>パイプライン ステージ
 ...
 [パイプライン ステージ](graphics-pipeline-stages.md)ウィンドウ視覚化個別に各ステージの結果されていない、レンダリングの問題が最初に表示されますので、どのステージをより簡単に判断できます。 それがどのステージであるかを判別した後は、[パイブライン ステージ] ウィンドウのその場所からシェーダーのデバッグを開始できます。
