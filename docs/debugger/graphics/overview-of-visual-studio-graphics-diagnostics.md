...
## <a name="directx-version-compatibility"></a>DirectX のバージョンの互換性
 グラフィックス診断は、Direct3D 10 以上を使用するアプリをサポートし、Direct2D を使用するアプリに対して限定的なサポートを提供します。 旧バージョンの Direct3D、DirectDraw、または他のグラフィックス API を使用するアプリケーションはサポートしません。

### <a name="windows-10-and-direct3d-12"></a>Windows 10 と Direct3D 12
 Windows 10 では direct3d *12*が導入されました。これは、direct3d 10 と direct3d 11 とは大きく異なります。 これらの違いは、DirectX を最新のグラフィックス ハードウェアに合わせて調整し、その潜在能力を十分に引き出すために生じたものですが、それによって API にも大幅な変更が必要となり、リソースの有効期間と競合を管理する仕事がこれまで以上にプログラマの責任になりました。 違いにかかわらず、Direct3D 12 とグラフィックス診断は、Direct3D 11.2 でグラフィックス診断との機能パリティを維持します。

 Windows 10 では、前のバージョンの Direct3D と、それらに依存するゲームやアプリケーションも引き続きサポートされます。 Visual Studio のグラフィックス診断では、Windows 10 の Direct3D 10 と Direct3D 11 が引き続きサポートされます。
...
## <a name="graphics-analyzer"></a>Graphics Analyzer
 ...
 ![すべてのグラフィックスデバッガーウィンドウ](media/graphicsdebuggerwindows.png "GraphicsDebuggerWindows")
...
### <a name="event-list"></a>イベント一覧
 ...
 [イベント一覧](graphics-event-list.md)には、調査中のフレーム中に記録されたすべてのグラフィックスイベントが表示されます。 重要な要素を見つけやすくするため、イベント一覧は階層的に表示することができます。たとえば、後続の描画呼び出しの下に最新の状態変化を表示したり、タイムラインとして表示したりできます。 さらに、イベントはどのキューに属しているかに基づいて色分け表示され、調査したいイベントだけを表示するように一覧をフィルター処理することもできます。
...
### <a name="pipeline-stages"></a>パイプライン ステージ
 ...
 [[パイプラインステージ](graphics-pipeline-stages.md)] ウィンドウでは、各ステージの結果が個別に視覚化されます。これにより、レンダリングの問題が最初に表示される段階を簡単に判断できるようになります。 それがどのステージであるかを判別した後は、[バイブライン ステージ] ウィンドウのその場所からシェーダーのデバッグを開始できます。
...
### <a name="pixel-history"></a>ピクセル履歴
 ...
 [[ピクセル履歴](graphics-pixel-history.md)] ウィンドウは、拒否された投稿を含め、フレーム内のすべてのピクセルのシェーダー履歴全体を視覚化します。 適用を拒否された色については、未加工のシェーダーの結果と、以前の色に新しい各色が結合される様子が表示されます。 この情報を見ると、シェーダーの結果を混ぜ合わせたピクセルの間違いの原因を比較的簡単に見つけたり、レンダリングされたオブジェクトが欠落している原因がピクセルへの色の適用を不適切に拒否されたことにあると判別したりできます。
...
### <a name="event-call-stack"></a>イベント呼び出し履歴
 ...
 [[イベント呼び出し履歴](graphics-event-call-stack.md)] ウィンドウには、イベント一覧のすべてのグラフィックスイベントの完全な呼び出し履歴が表示されます。また、デバッグ情報が利用可能な場合は、アプリのソースコードに移動することもできます。 これは、エラーが最初に発見された GPU 上の場所から跡をたどり、アプリのソース コードに戻って問題の発生場所を突き止めることのできる強力なツールです。
