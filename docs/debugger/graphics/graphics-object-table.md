---
title: グラフィックスオブジェクトテーブル |Microsoft Docs
ms.date: 11/04/2016
ms.topic: conceptual
f1_keywords:
...
## <a name="understanding-the-graphics-object-table"></a>グラフィックス オブジェクト テーブルについて
 オブジェクト テーブルを使用すると、特定のフレームのレンダリングをサポートする Direct3D のオブジェクトを解析できます。 プロパティとデータを調べることによって、特定のオブジェクトに対するレンダリングの問題を特定することができます (診断の前に他のグラフィックス診断ツールを使用すると、予期していないオブジェクトの一覧を絞り込むことができます)。問題のあるオブジェクトが見つかったら、その型に固有の視覚エフェクトを使用してそれを調べることができます。たとえば、イメージエディターを使用してテクスチャを表示したり、*バッファービジュアライザー*を使用してバッファーの内容を表示したりできます。

 オブジェクト テーブルは、コピーして貼り付ける操作をサポートしているため、Microsoft Excel などの別のツールを使用してその内容を調べることができます。

 また、左上隅にある **[型]** ドロップダウンを使用して、**バッファー**、**シェーダー** 、または**テクスチャ**の型のオブジェクト、またはこれらの項目のすべてを一度に表示することもできます。  また、右上隅の [検索] ボックスを使用して、表示されるすべてのデータについて特定の行を検索することもできます。  たとえば、 *D32_FLOAT*を検索して、一覧内のその形式のオブジェクトのすべてのインスタンスを検索することができます。
...
## <a name="graphics-object-viewers"></a>グラフィックス オブジェクト ビューアー
 ...
 ![出力合併を表示するテクスチャプレビュー](media/gfx_diag_texture_preview.png "gfx_diag_texture_preview")
...
### <a name="device-context-object"></a>デバイス コンテキスト オブジェクト
 Direct3D 11 および Direct3D 10 では、デバイス コンテキスト (**D3D11 Device Context** または **D3D10 Device**) オブジェクトは、最も重要な状態情報を保持しているため特に重要であり、現在設定されている他の状態オブジェクトにリンクされます。 デバイスコンテキストの詳細は、新しいドキュメントウィンドウに表示されます。各カテゴリの情報は、それぞれのタブに表示されます。デバイスコンテキストは、現在のデバイスの状態を反映するために新しいイベントが選択されたときに変更されます。
