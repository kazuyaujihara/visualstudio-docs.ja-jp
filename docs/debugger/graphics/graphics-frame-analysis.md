---
title: グラフィックスフレーム分析 |Microsoft Docs
ms.date: 02/09/2017
ms.topic: conceptual
f1_keywords:
...
- フレーム分析は、以前にキャプチャした情報から新しいパフォーマンス サマリーを生成できる (例: GPU ドライバーが最適化された、または追加のデバッグ機能を公開した場合など)。
  ...
  アプリに対して実行できるフレーム分析のデモンストレーションについては、Channel 9 の[Visual Studio グラフィックスフレーム分析](https://channel9.msdn.com/Shows/C9-GoingNative/GoingNative-25-Offline-Analysis-Graphics-Tool)ビデオをご覧ください。
...
#### <a name="timeline"></a>タイムライン
 ...
 ![タイムラインには&#45;描画呼び出しのコストが表示されます。](media/pix_frame_analysis_timeline.png "pix_frame_analysis_timeline")
...
#### <a name="table"></a>テーブル
 ...
 ![概要テーブルには、さまざまなバリアントが表示されます。](media/pix_frame_analysis_summary.png "pix_frame_analysis_summary")
...
#### <a name="hot-draw-calls"></a>"ホット" 描画呼び出し
 ...
 ![この DrawIndexed の呼び出しには、ホットバリアントとコールドバリアントがあります。](media/pix_frame_analysis_hot_calls.png "pix_frame_analysis_hot_calls")
...
#### <a name="statistical-significance"></a>統計的な有意性
 ...
 ![描画呼び出しバリアントの統計的関連性](media/pix_frame_analysis_summary_stats.png "pix_frame_analysis_summary_stats")

 統計的な関連性を判断するために、フレーム分析は[学生の t 検定](https://en.wikipedia.org/wiki/Student's_t-test)を使用します。
...
#### <a name="platforms-that-do-not-support-hardware-counters"></a>ハードウェア カウンターをサポートしていないプラットフォーム
 ...
 ![詳細テーブルといくつかの再生バリエーション。](media/pix_frame_analysis_details.png "pix_frame_analysis_details")
...
#### <a name="platforms-that-support-hardware-counters"></a>ハードウェア カウンターをサポートしているプラットフォーム
 ...
 ![サポートされている場合は、ハードウェアカウンターが表示されます。](media/pix_frame.png "pix_frame")
...
### <a name="gpu-counters"></a>GPU カウンター
 ...
 現在 Intel、AMD、または nVidia で提供されているコンピューター用 GPU で、GPU ハードウェア カウンターを確実にサポートしているものがないため、フレーム分析はこれらの GPU からカウンターを収集しません。 ただし、フレーム分析は次の GPU からハードウェアカウンターを収集し、それらを確実にサポートします。
...
> [!NOTE]
> 機能レベルの問題に留意しておくことは重要ですが、異なるハードウェア構成およびデバイス上でもグラフィックス ログ ファイルをキャプチャおよび再生できます。 ログファイルに Api が含まれていない限り、または再生コンピューターでサポートされていない機能レベルを使用している場合は、グラフィックスログを再生できます。
...
### <a name="warp"></a>WARP
 フレーム分析は、実際のハードウェアでレンダリング パフォーマンスをプロファイルおよび改善する目的で使用されます。 ワープデバイスでフレーム分析を実行することは禁止されていませんが、通常、ハイエンド CPU で実行されているワープは、最も性能の低い最新の Gpu よりも低速であり、ワープのパフォーマンスは特定の CPU によって大きく異なる可能性があるため、通常は価値のあるものではありません。これはで実行されています。
...
|**ポイント テクスチャ フィルタリング**|該当するすべてのテクスチャ サンプルに対して、フィルタリング モードを `DXD11_FILTER_MIN_MAG_MIP_POINT` (point texture filtering) に設定します。<br /><br /> 詳細については、「ポイント」、「バイリニア」、「[トライリニア」、および「異方](point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md)」を参照してください。|
|**バイリニア テクスチャ フィルタリング**|該当するすべてのテクスチャ サンプルに対して、フィルタリング モードを `DXD11_FILTER_MIN_MAG_LINEAR_MIP_POINT` (bilinear texture filtering) に設定します。<br /><br /> 詳細については、「ポイント」、「バイリニア」、「[トライリニア」、および「異方](point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md)」を参照してください。|
|**トリリニア テクスチャ フィルタリング**|該当するすべてのテクスチャ サンプルに対して、フィルタリング モードを `DXD11_FILTER_MIN_MAG_MIP_LINEAR` (trilinear texture filtering) に設定します。<br /><br /> 詳細については、「ポイント」、「バイリニア」、「[トライリニア」、および「異方](point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md)」を参照してください。|
|**アニソトロピック テクスチャ フィルタリング**|該当するすべてのテクスチャ サンプルに対して、フィルタリング モードを `DXD11_FILTER_ANISOTROPIC` に設定し、`MaxAnisotropy` を `16` (16x アニソトロピック テクスチャ フィルタリング) に設定します。<br /><br /> 詳細については、「ポイント」、「バイリニア」、「[トライリニア」、および「異方](point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md)」を参照してください。|
