...
計算とテキスト ブロックの混在を回避しようとしてください。 各テキスト テンプレート内で 1 つ目を使用して、 \<# コード ブロックの #> を変数を設定し、複雑な計算を実行します。 テンプレートまたは 1 つ目の末尾までの最初のテキスト ブロックから\<#+ クラス機能ブロックの #> 長い式を避けるため、テキスト ブロックがない限り、ループや条件を回避します。 この方法により、テンプレートは読みやすくなりメンテナンスが簡単になります。
