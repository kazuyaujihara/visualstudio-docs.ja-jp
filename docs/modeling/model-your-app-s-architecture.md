...
- 基に、データベースとファイル システム、ネットワーク対応のアプリケーションと web クライアント間で choice と選択などのテクノロジの選択しなど。
...
## <a name="Patterns"></a> 設計パターン
 ...
 よく知られていて、広く適用されている、Observer などの汎用的なデザイン パターンがいくつかあります。 さらに、特定のプロジェクトにのみ適用可能なパターンもあります。 たとえば、web 販売システムがあります、コードのいくつかの操作、顧客の注文に変更が行われる場所。 注文の状態がどの段階においても正確に表示されるように、これらのすべての操作は、データベースを更新するための特定のプロトコルに従う必要があります。
