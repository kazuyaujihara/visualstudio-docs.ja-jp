...
このトピックでは、依存関係図で使用できる要素について説明します。 依存関係図を作成して描画する方法の詳細につい[ては、「依存関係図」を参照してください。ガイドライン](../modeling/layer-diagrams-guidelines.md)。 レイヤーパターンの詳細については、[パターン & プラクティス](http://go.microsoft.com/fwlink/?LinkId=145794)に関するサイトを参照してください。
...
|2|**関係**|あるレイヤーが別のレイヤーの機能を使用することはできても、その逆はできないことを示します。<br /><br /> -   **Direction** -依存関係の方向を指定します。|
...
|**検証をサポート**|**True**の場合、レイヤー検証プロセスは、プロジェクトがこの要素との依存関係に準拠しているかどうかを検証できます。<br /><br /> **False**の場合、リンクはレイヤー検証プロセスに参加しません。<br /><br /> 詳細については[、「依存関係図」を参照してください。ガイドライン](../modeling/layer-diagrams-guidelines.md)。|
|**識別子**|リンクされた成果物への参照|
