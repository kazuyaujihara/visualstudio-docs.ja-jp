...
|削除を伝達します。|`True` 関連のリンクが削除されたときにこの役割を果たす要素を削除します。|`True` 埋め込みのロールのターゲット。<br /><br /> `False` その他のロール。|
...
|Category|生成されたプロパティを表示するカテゴリ、**プロパティ**生成されたデザイナーでウィンドウ。 かどうかは、このプロパティは空では、下に、生成されたプロパティが表示されます、 **Misc**カテゴリ|< なし\>|
