...
|`ThisWorkbook`、 `Sheet` *n*、 `ThisDocument`、および`ThisAddIn` といったホスト項目クラスは、オーバーライド可能なする `Dispose` メソッドは提供されなくなります。|例えば、ホスト項目クラス中の `Shutdown` イベント ハンドラーをオーバーライドしている `Dispose` メソッド中のすべてのコードを、`ThisAddIn_Shutdown` に移動し、ホスト項目クラスから `Dispose` メソッドのオーバーライドを削除する必要があります。|  
  
## <a name="see-also"></a>関連項目  
 [.NET Framework 4 またはそれ以降の Office ソリューションの移行](../vsto/migrating-office-solutions-to-the-dotnet-framework-4-or-later.md)   
