---
title: '[発行] ページ、プロジェクト デザイナー (Office 開発)'
ms.date: 02/02/2017
ms.topic: conceptual
f1_keywords:
 ...
 **発行フォルダーの場所 (web サイト、ftp サーバー、またはファイル パス)** 必要です。
 ...
 オプションには、ローカル コンピューター、UNC ファイル共有、または HTTP/HTTPS Web サイトがあります。 ローカル パスを指定できます (*c:\foldername\publishfolder*)、相対 (*発行\\* )、または完全修飾の場所 ( *\\\servername\foldername*または http://<em>servername/foldername</em>)。

 発行場所は、既定では、 *http://localhost/projectname/* 、IIS をインストールした場合、または*発行\\* ディレクトリを IIS がインストールされていない場合。

 **インストール フォルダーの URL**省略可能です。
 ...
 オプションには、ローカル コンピューター、UNC ファイル共有、または HTTP/HTTPS Web サイトがあります。 ローカル パスを指定できます (*c:\foldername\publishfolder*)、相対 (*発行\\* )、または完全修飾の場所 ( *\\\servername\foldername*または http://<em>servername/foldername</em>)。 HTTP/HTTPS の場所は、すべて US-ASCII 文字で作成する必要があります。 Unicode 文字はサポートされていません。
 ...
 この値は、管理者が後で割り当てることができます。 詳細については、「[方法 :Office ソリューションのインストール パスを変更して](https://msdn.microsoft.com/d0eaa07b-2d72-4902-899f-2f9fb165b8fd)します。

 **前提条件**の前提条件のセットアップ プログラムに含まれるまたはインストール時にオンデマンドでダウンロードできます。

- **必須コンポーネントのベンダーの web サイトからダウンロードする**:これらの前提条件を Microsoft からダウンロードするのにには、このオプションを使用します。

- **アプリケーションと同じ場所から必須コンポーネントをダウンロード**:必須コンポーネントをインストーラーにパッケージ化するのにには、このオプションを使用します。 必須コンポーネントのファイルをセットアップ プログラムに含めると、ソリューションのサイズが大きくなります。

- **次の場所から必須コンポーネントをダウンロード**:このオプションを使用して、web ページまたはネットワーク共有上の別のセットアップ プログラムとは別に前提条件を用意をエンドユーザーが利用できるようにします。

  **更新プログラム**更新間隔は、ソリューションが更新プログラムをチェックする頻度を決定します。 既定では、7 日ごとに確認するように設定されます。
  ...
  **オプション (説明)** 次のプロパティの発行オプションを設定することができます。
...
- サポート URL: エンド ユーザーが Office ソリューションのテクニカル サポートに連絡するための場所です。
  ...
  **オプション (Office の設定)** 次のプロパティの発行オプションを設定することができます。
...
- [VSTO アドインの読み込み動作]
  ...
  **発行の言語**このオプションは、Microsoft ソフトウェア ライセンス条項の言語を設定し、前提条件の一覧で、言語パックが含まれています。 カスタマイズの言語には影響しません。 セットアップ プログラムの言語は、インストールされている Visual Studio の言語によって決まります。
  ...
  詳細を変更する方法については、**発行の言語**を参照してください[方法。ClickOnce アプリケーションの発行の言語を変更する](../deployment/how-to-change-the-publish-language-for-a-clickonce-application.md)します。

  **発行バージョン**カスタマイズのバージョン番号を設定します。 バージョン番号を変更すると、アプリケーションが更新プログラムとして発行されます。 ビルド プロセス時に各バージョン用の新しいフォルダーが作成されるため、前に発行されたバージョンは上書きされません。 発行バージョンを構成する各部分 ( **[メジャー]** 、 **[マイナー]** 、 **[ビルド]** 、 **[リビジョン]** ) には、それぞれ 5 桁までの数値を含めることができます。

  **リリースごとにリビジョンを自動的にインクリメント**(省略可能)。 このオプションが選択されている場合 (既定)、バージョン番号の **[リビジョン]** 部分は、カスタマイズが発行されるたびに 1 ずつインクリメントされます。 これにより、そのカスタマイズが更新プログラムとして発行されます。
