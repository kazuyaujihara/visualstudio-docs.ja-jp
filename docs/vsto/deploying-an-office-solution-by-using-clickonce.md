...
## <a name="Helping"></a> ユーザー ソリューションのインストールの支援します。
 ユーザー ソリューションをインストール、またはドキュメント レベルのカスタマイズ中に、配置マニフェストを開いて、セットアップ プログラムを実行して、ドキュメントを直接開きます。 ベスト プラクティスとして、ユーザーはセットアップ プログラムを使用してソリューションをインストールする必要があります。 その他の 2 つの方法はない前提条件のソフトウェアがインストールされていることを確認します。 ユーザーがインストール場所からドキュメントを開こうとする場合は、Office アプリケーションのセキュリティ センターにある信頼できる場所の一覧に、そのインストール場所を追加する必要があります。
...
## <a name="Put"></a> ソリューションのドキュメントをエンドユーザーのコンピューター (ドキュメント レベルのカスタマイズのみ) に配置します。
 ...
 次の手順は、プロジェクト名が前提としています。 **ExcelWorkbook**という名前の作成したフォルダーにソリューションを発行することと**C:\publish**コンピューターにします。
...
1. ソリューションのディレクトリを開く**c:\publish**を使用して**ファイル エクスプ ローラー**します。
...
## <a name="Custom"></a> カスタム インストーラーを作成します。
 カスタム インストーラーは、ソリューションを発行するときに作成するセットアップ プログラムを使用する代わりに、Office ソリューションを作成できます。 たとえば、スクリプトでインストールを開始する、符号を使用する可能性があります。 またはバッチ ファイルを使用して、ユーザーの介入なしにソリューションをインストールする可能性があります。 このようなシナリオは、エンド ユーザーのコンピューターに必須コンポーネントがインストール済みの場合に最適です。
...
|-100|1 つまたは複数のコマンド ライン オプションは無効または複数回設定されました。 詳細については、次のように入力します"vstoinstaller/でしょうか。"。 参照してくださいまたは[ClickOnce Office ソリューション用のカスタム インストーラーを作成する](https://msdn.microsoft.com/3e5887ed-155f-485d-b8f6-3c02c074085e)します。|
|-101|1 つまたは複数のコマンド ライン オプションは無効です。 詳細については、"vstoinstaller /?" と入力してください。|
