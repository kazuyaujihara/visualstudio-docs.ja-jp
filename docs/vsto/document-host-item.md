...
### <a name="limitations-of-the-document-host-item-in-document-level-projects"></a>ドキュメントレベルのプロジェクトのドキュメントホスト項目の制限事項
 ...
 実行時に新しい Word ドキュメントを作成すると、そのワークシートは <xref:Microsoft.Office.Interop.Word.Document>型になります。 これはホスト項目ではないため、ホスト コントロールや Windows フォーム コントロールを含めることはできません。 実行時にドキュメントを作成する方法の詳細に[ついては、「」を参照してください。プログラムによって](../vsto/how-to-programmatically-create-new-documents.md)新しいドキュメントを作成します。
