...
すべてのスイッチは 2 つの形式 (-switch および /switch) で使用可能です。 このドキュメントでは、-switch の形式のみを示しています。
